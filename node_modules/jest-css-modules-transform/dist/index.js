...
const postcssNested = postcss_1.default([postcss_nested_1.default]);
...
const getGlobalSassData = (rootDir) => {
    try {
        return `${require(path_1.resolve(rootDir, '.sassrc.js')).data}\n` || '';
    }
    catch (e) {
        return '';
    }
...
const lessPath = path_1.resolve(__dirname, 'less.js');
const postcssPath = path_1.resolve(__dirname, 'postcss.js');
...
const moduleTransform = {
    process(src, path, transformConfig) {
        ...
        getFileData = getFileData || utils_parser_1.createFileCache(config.cwd);
        configPath = configPath || path_1.resolve(config.rootDir, CONFIG_PATH);
        preProcessorsConfig = preProcessorsConfig || utils_1.getPreProcessorsConfig(configPath);
        ...
        if (preProcessorsConfig.prepend) {
            const urls = utils_parser_1.extractUrls(preProcessorsConfig.prepend, path);
        ...
        switch (extention) {
            ...
            case 'scss':
                ...
                break;
            case 'less':
                textCSS = child_process_1.execSync(`node ${lessPath} ${path} ${configPath} ${prependDataContent ? 1 : 0}`, {
                    ...nodeExecOptions,
            ...
            case 'pcss':
            case 'postcss':
                postcssConfigPath = '' || path_1.resolve(config.rootDir, 'postcss.config.js');
                postcssConfig =
                    postcssConfig ||
                        preProcessorsConfig.postcssConfig ||
                        utils_1.requirePostcssConfig(postcssConfigPath);
                if (postcssConfig) {
                    textCSS = child_process_1.execSync(`node ${postcssPath} ${path} ${configPath} ${postcssConfigPath} ${prependDataContent ? 1 : 0}`, {
                        ...nodeExecOptions,
                        input: prependDataContent,
                    });
                }
